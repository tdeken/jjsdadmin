...
import { FormSchema, InitForm } from './form';
...
const onSubmit = async (values: Record<string, any>) => {
    ...
    await goodsSkuCreate({
      goods_id: drawerApi.getData().goods_id,
...
const [Drawer, drawerApi] = useVbenDrawer({
  ...
  onOpenChange: (isOpen) => {
    if (isOpen) {
      InitForm(COPY, drawerApi, formApi)
    }
  },
});

const [Form, formApi] = vbenForm(FormSchema(COPY), onSubmit);
